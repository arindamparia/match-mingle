    ...
    private void updateUserLockStatus(@Valid IdRequest idRequest, boolean lockStatus) {
        try {
            ...
            if (user.getEmail().equals(loggedInUserEmail)) {
                String errorMessage = lockStatus ? Constants.CANNOT_SELF_LOCK : Constants.CANNOT_SELF_UNLOCK;
                throw new IllegalArgumentException(errorMessage);
            }

            boolean currentLockStatus = Boolean.TRUE.equals(user.getUserLocked());
            if (currentLockStatus == lockStatus) {
                String errorMessage = lockStatus ? Constants.USER_ALREADY_LOCKED : Constants.USER_ALREADY_UNLOCKED;
                throw new IllegalArgumentException(errorMessage);
            }
            ...
            String errorMessage = String.format("Error occurred while %s user: %s", action, idRequest.getId());
